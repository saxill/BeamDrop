...
```bash
# on the laptop
GOOS=linux GOARCH=arm64 go build -o beamdrop-pi ./cmd/beamdrop   # 64-bit Pi OS
scp beamdrop-pi pi@raspberrypi:~/beamdrop
```

On the Pi: join the tailnet (`tailscale up`), then run it as a service so it
survives reboots:

```ini
# ~/.config/systemd/user/beamdrop.service
[Unit]
Description=beamdrop relay
After=network-online.target

[Service]
ExecStart=%h/beamdrop portal --relay
Restart=always
RestartSec=5
